...
- 注意: 仕組み上、キーを知っている人は Web でも全機能を使えます。この価格帯の学習アプリとしては十分な抑止と判断しました。
...
3. 「収益化」→ 「アプリの価格」→ 有料、価格 3,000 円（App Store と同じ）。配信国に日本を追加。
4. 「テスト」→ 「内部テスト」→ 新しいリリース → `.aab` をアップロード → リリースノートを書いて公開。テスターに自分のアカウントを追加し、実機でインストールして確認する（TWA でアドレスバーが出ないこと、キー付き起動で全機能になること、オフライン動作）。人数の制限はなく、数分で入る。
5. **【重要】個人の開発者アカウントは、製品版の前にクローズドテストが要る**（2023 年 11 月以降に作ったアカウントが対象）。条件は次の 3 つで、Play Console のダッシュボードに進み具合が出る。
   - クローズドテスト版のリリースを公開する
   - **12 人以上**のテスター（Google アカウント）にオプトインしてもらう
   - その状態を**連続 14 日間**続ける（途中で抜けると数え直し）
   テスターはメールアドレスの一覧か Google グループで登録する。有料設定でもテスターは無料で入れられる。
6. 条件を満たすと「製品版へのアクセスの申請」が押せるようになる。アンケート（テストで分かったこと、対象の利用者など）に答えて申請 → 承認後に「製品版」→ 新しいリリース → 同じ `.aab` を選んで審査に送る。初回審査は数日〜1 週間程度。ここまで最短で 2〜3 週間かかる。

## 3b. 進み具合（2026-09-03 時点）
- パッケージ作成: 済み（`kokuteki-private/play/TENRYU.aab`、Package ID `com.amaterasuvocab.kokuteki`、Start URL に鍵入り）
- 署名鍵のフィンガープリント: assetlinks.json に反映して公開済み
- 残り: 内部テストへアップロード → **Play の再署名鍵の SHA-256 を assetlinks.json に追加** → アプリの設定（掲載情報・データセーフティなど）→ クローズドテスト 12 人 14 日 → 製品版の申請
