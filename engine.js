...
<path fill-rule="evenodd" d="M0,0 H360 V240 H0 Z M16,40 Q180,4 344,40 L344,182 L16,182 Z" fill="var(--bezel, #0a0d11)"/>${ckFrame(hud)}</svg>`;
  ...
  function figTopDown(theta, phi, mark = 0) {
    ...
    /* 印（三角と文字）と機体の絵が重ならないように、機体を 144 → 108 に縮めて文字の内側に収める。印は絵の上に描き、文字に背景色の縁取り
       （尾翼や翼が印の方向にあると、文字が機体の下に隠れていた。利用者の指摘 2026-09-14）。文字は印といっしょに回さず正立（下にある SW が「MS」と読めた） */
...
<g transform="rotate(${phi} 100 100)"><polygon points="100,13 94,27 106,27" fill="var(--accent)"/><text x="100" y="41" transform="rotate(${-phi} 100 36)" text-anchor="middle" font-family="var(--mono)" font-size="14" font-weight="700" fill="var(--accent)" stroke="var(--card)" stroke-width="4" stroke-linejoin="round" paint-order="stroke">${DIRS[mark].k}</text></g></svg>`;
  }
