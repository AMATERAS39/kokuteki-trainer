...
(function (g) {
  ...
  const EXAM = { V: 200, INPUT: { 'stick-right': { x: 0.17 }, 'stick-left': { x: -0.17 }, 'stick-forward': { y: 0.0728 }, 'stick-back': { y: -0.0483 }, 'rudder-right': { r: 0.5259 }, 'rudder-left': { r: -0.5259 },
    /* 見え方の一覧だけの斜め（おまけ）: 左右と奥手前の同時。出題には無い */
    'stick-forward-left': { x: -0.17, y: 0.0728 }, 'stick-forward-right': { x: 0.17, y: 0.0728 }, 'stick-back-left': { x: -0.17, y: -0.0483 }, 'stick-back-right': { x: 0.17, y: -0.0483 } } };
  ...
  const WORLD = {
    ...
    /* 地面の格子: 真上から見て正方形の格子（東西・南北の線を gap m おき、始めの位置のまわり ±gridR m）。写すと遠近で消失点へ向かって見える。
       v05.29 までは①の水平飛行の画面の中央へ向かう「放射の線」（不等間隔 12 本）と横の線で、真上から見ると格子になっていなかった（利用者の指摘 2026-09-14） */
    gap: 300, gridR: 12000,   // 遠くまで引くと水平線の近くで線が詰まって暗い帯になるので、出題の絵と 3D で同じ ±12 km
