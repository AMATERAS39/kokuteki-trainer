...
export function mount(container, opt = {}) {
  ...
  /* サンライズの散開（v04.30）: 地面と平行な面の上で開く。隊形の間隔を広げるのではなく、機体それぞれが短く旋回して
     ...
     （4 秒の旋回で 66.7 度 → 60 度。内側は 1.5 秒遅れて開くので 15.5 度 → 12.4 度。数で確かめた） */
  const BLOOM_IN_ANG = 15.5, BLOOM_OUT_ANG = 66.7;
  ...
  /* 外側の 2 機が先に開き、内側の 2 機は 1.5 秒遅れて開く（v04.32。同時に開くと、開き始めに内側が凹んで見える。利用者の指摘）。
