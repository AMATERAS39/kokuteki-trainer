...
export function mount(container, opt = {}) {
    ...
    switch (m.id) {
      ...
      case 'tuck': {
        ...
        if (toC > TUCK_ROLL_D) {
          ...
          /* 戻りの段（tkWp === 1）だけ、線に近づくほど強く引く。
             steerTo は高さ合わせでピッチを決めるので、そのあとに上書きする。
             n0 は線からの離れ（負の側から 0 へ戻ってくる） */
          if (tkWp === 1) {
            const pull = clamp((TUCK_PULL_N + n0) / TUCK_PULL_N, 0, 1);
            if (pull > 0) autoIn.y = -TUCK_PULL * pull;
          }
  ...
  /* 交差の直前の引き起こし（利用者の提案 2026-09-12）。線へ戻るあいだ、機体は線の側へ深く傾いていて、
     上面が交差点の側を向いている。そこで操縦桿を引くと、機首が交差点へ向けて振れて動きにキレが出る。
     この飛行モデルでは位置を機首の向きに沿って積むので、曲がりを詰める効果もある。
     TUCK_PULL_N: 線までの残り何 m から引き始めるか。TUCK_PULL: 引きの強さ（1 で一杯） */
  const TUCK_PULL_N = 55, TUCK_PULL = 0.8;
  /* 外へロールしきるまで（秒）。1.2 → 1.0 → 0.6 → 1.0 に戻した。
     0.6 秒は速すぎて、利用者の見立てでは 1.0 秒のほうがよかった（2026-09-12「一つ前の方がよかった」）。
     実機は操縦桿の横と方向舵で回して、上面を交差点の側へ向ける。
     この模擬では姿勢を直接与えている区間なので、所要時間で速さを合わせる */
  const TUCK_ROLL_T = 1.0;
